...
PAYLOADS = {
    "collections": lambda: _collections_dashboard(collections_snapshot()),
    "treasury": lambda: _treasury_dashboard(treasury_baseline()),
    "finance": lambda: _finance_dashboard(finance_snapshot()),
    "leakage": lambda: _leakage_dashboard(leakage_snapshot()),
}
...
def registry_keys() -> dict[str, set[str]]:
    ...
    for agent in ("finance", "treasury", "collections", "leakage"):
        match = re.search(rf"\n  {agent}: \{{(.*?)\n  \}},?\n", body, re.S)
...
def test_registry_is_valid_and_covers_four_agents() -> None:
    keys = registry_keys()
    assert set(keys) == {"finance", "treasury", "collections", "leakage"}
